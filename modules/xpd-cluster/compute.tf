...
resource "oci_core_instance" "bm_nodes" {
  count      = local.use_compute_cluster_mode ? var.memory_node_count : 0
...
resource "oci_core_instance_console_connection" "bm_console" {
  count = var.create_bm_console_connections && local.use_compute_cluster_mode ? var.memory_node_count : 0
